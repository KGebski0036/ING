...
def group_and_sort_within_group(input_csv, group_column_index, sort_column_index, extract_columns):
    ...
    df_grouped = df.groupby(df[group_column_index])
    ...
    extracted_data[extract_columns[0]] = extracted_data[extract_columns[0]].fillna("")

    # Convert to lists for payloads and labels
    payloads = extracted_data[extract_columns[0]].tolist()
    labels = extracted_data[extract_columns[1]].tolist()
